...
#   return HRTSResult(:infeasible, [(start, "N", 0.)], NaN, NaN, NaN, nothing)
  ...
  while !isempty(hrts_state.openheap)
    ...
    curNode = FocalPop(hrts_state.openheap, w_param)
    ...
    # hrts_state.openheap = openheap_mod
    ...
    # if feasAE
      ...
      println("Analytical Expn from node: $(curNode.config), RiskIntegral: $(curNode.intgrCons)")
      println("node parent: $(curNode.parent.config), RiskIntegral: $(curNode.parent.intgrCons)")
...
function FocalPop(opnheap, w)
  ...
  for nd in opnheap[inds]
    ...
    push!(focalSet_ids, nd.id)
    push!(focalSet_intgrCons, nd.intgrCons)

  end
  returnNodeId = focalSet_ids[argmin(focalSet_intgrCons)]

  popNode = opnheap[findall(x->x.id == returnNodeId, opnheap )]
  deleteat!(opnheap, findall(x->x.id == returnNodeId, opnheap ))
  heapify!(opnheap)

  # println("minLabelCost: ", minLabelCost)
  # println("cost of pop node: ", popNode[1].f)
